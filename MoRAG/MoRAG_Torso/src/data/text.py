...
def load_gpt_texts(path, name="torso.json"):
    json_path = os.path.join(path, name)
    return load_json(json_path)
...
def save_token_embeddings(
    ...
    gpt_texts = load_gpt_texts("datasets/annotations/humanml3d/")
    ...
    for dico in annotations.values():
        for lst in dico["annotations"]:
            all_texts.append(gpt_texts[lst["text"]])
